...
"[project]/.next-internal/server/app/api/projects/[id]/scans/[scanId]/route/actions.js [app-rsc] (server actions loader, ecmascript)": (function(__turbopack_context__) {
...
"[project]/src/app/api/projects/[id]/scans/[scanId]/route.ts [app-route] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
async function GET(request, { params }) {
    try {
        ...
        const { data: scan, error: scanError } = await supabase.from("scans").select("*").eq("id", scanId).eq("project_id", projectId).single();
        if (scanError) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        ...
        const { count: vulnerabilitiesCount, error: countError } = await supabase.from("vulnerabilities").select("*", {
            count: "exact",
            head: true
        }).eq("scan_id", scanId);
        if (countError) {
        ...
        if (scan.status === "running") {
            ...
            if (scan.progress_percentage === null || scan.progress_percentage === undefined) {
                ...
                scan.progress = Math.min(Math.floor(elapsed / estimatedDuration * 100), 99);
            } else {
...
async function PATCH(request, { params }) {
    try {
        ...
        const { error: checkError } = await supabase.from("scans").select("id").eq("project_id", projectId).eq("id", scanId).single();
        if (checkError) {
        ...
        // Update the scan
        const { data, error } = await supabase.from("scans").update({
            ...updates,
            updated_at: new Date().toISOString()
        }).eq("id", scanId).select().single();
        if (error) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__fadf457c._.js.map